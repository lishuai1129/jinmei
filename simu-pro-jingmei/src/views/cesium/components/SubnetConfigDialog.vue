                  ...
                  <div class="tab-content-wrapper">
                    ...
                    <template v-else-if="selectedModel === 'rfpipe'">
                      <div class="config-subsection">
                        ...
                        <div class="form-row">
                          <div class="form-label">延迟 (ms)</div>
                          <el-input-number
                            v-model.number="configs.mac.delay"
                            :min="0"
                            :step="0.1"
                            :precision="1"
                            class="form-input"
              ...
              if (param && param.group) {
                ...
                if (param.group === 'External Parameters') {
                  ...
                  if (key in configs.mac) {
                    ...
                    // 如果是delay参数，从秒转换为毫秒，并加回5ms，然后转换为数字类型
                    if (key === 'delay' && !isNaN(parseFloat(value))) {
                      // 秒转毫秒，加回5ms（因为保存时减少了5ms）
                      (configs.mac as any)[key] = parseFloat(value) * 1000 + 5;
                    } else {
...
const handleConfirm = async () => {
  try {
    ...
    // 确保所有数值参数都是字符串类型
    // 同时处理延迟单位转换：延迟从毫秒转换为秒（EMANE使用秒）
    Object.keys(configs.mac).forEach((key) => {
      if (configs.mac[key as keyof typeof configs.mac] !== undefined) {
        let value = configs.mac[key as keyof typeof configs.mac];

        // 如果是delay参数，自动减少5ms后再从毫秒转换为秒
        if (key === "delay" && typeof value === "number") {
          // 减少5ms后再转换为秒，保留6位小数，但不能小于0
          value = (Math.max(0, (value - 5) / 1000)).toFixed(6);
        }

        configs.mac[key as keyof typeof configs.mac] = String(value);
      }
    });
