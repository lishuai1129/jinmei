...
function genRJ45Name() {
  const existing = (topoStore.topoData?.nodes || []).filter((n: any) => n.type === 'RJ45' || (typeof n.name === 'string' && n.name.startsWith('ens37')));
  if (existing.length === 0) return 'ens37';
  return `ens37_${existing.length + 1}`;
}
