...
export const useEmaneStore = defineStore('emane', {
  ...
  getters: {
    ...
    getRFSignalByNem: (state: EmaneState) => (nemId: number, viewpointNemId?: number): RFSignalTable | undefined => {
      console.log('[getRFSignalByNem查询]', 'nemId:', nemId, 'viewpointNemId:', viewpointNemId, 'viewpointData键:', Object.keys(state.viewpointData));
      ...
      if (viewpointNemId !== undefined && state.viewpointData[viewpointNemId]) {
        ...
        if (signal) {
          console.log('[getRFSignalByNem命中]', '从指定视角获取数据', 'signal:', signal);
          return signal;
        }
      }
        ...
        for (const [otherViewpointId, viewpoint] of Object.entries(state.viewpointData)) {
          if (parseInt(otherViewpointId) !== viewpointNemId) {
            ...
            if (signal) {
              ...
              console.log('[getRFSignalByNem命中]', '从其他视角获取数据', 'result:', result);
              return result;
            }
          }
        }
      ...
      // 如果没有指定视角NEM ID或者指定的视角没有数据，尝试从所有视角中查找
      console.log('[getRFSignalByNem搜索]', '在所有视角中搜索');
      for (const [vpId, viewpoint] of Object.entries(state.viewpointData)) {
        console.log('[getRFSignalByNem搜索细节]', 'viewpointId:', vpId, 'rfSignalTables长度:', viewpoint.rfSignalTables?.length || 0, 'rfSignalTables:', viewpoint.rfSignalTables);
        const signal = viewpoint.rfSignalTables.find(
          (s: RFSignalTable) => s.nem === nemId
        );
        if (signal) {
          console.log('[getRFSignalByNem命中]', '从所有视角查找命中', 'signal:', signal);
          return signal;
        }
      }

      console.log('[getRFSignalByNem未命中]', 'nemId:', nemId, '在所有数据中都找不到');
      return undefined;
    },
    ...
    updateFromWebSocket(wsData: any) {
      try {
        ...
        if (wsData.emaneInfo && wsData.emaneInfo.nemId !== undefined) {
          ...
          console.log('[EMANE数据接收]', '视角NEM ID:', viewpointNemId, 'RF表数量:', wsData.emaneInfo.rfSignalTables?.length || 0);
          console.log('[EMANE完整数据结构]', {
            neighborMetricTables: wsData.emaneInfo.neighborMetricTables?.length || 0,
            neighborStatusTables: wsData.emaneInfo.neighborStatusTables?.length || 0,
            rfSignalTables: wsData.emaneInfo.rfSignalTables?.length || 0
          });
          if (wsData.emaneInfo.rfSignalTables && wsData.emaneInfo.rfSignalTables.length > 0) {
            console.log('[EMANE-RFSignalTable示例]', wsData.emaneInfo.rfSignalTables[0]);
          }
          if (wsData.emaneInfo.neighborMetricTables && wsData.emaneInfo.neighborMetricTables.length > 0) {
            console.log('[EMANE-NeighborMetricTable示例]', wsData.emaneInfo.neighborMetricTables[0]);
          }
          if (wsData.emaneInfo.neighborStatusTables && wsData.emaneInfo.neighborStatusTables.length > 0) {
            console.log('[EMANE-NeighborStatusTable示例]', wsData.emaneInfo.neighborStatusTables[0]);
          }
          ...
          console.log('[EMANE数据已存储]', '视角NEM ID:', viewpointNemId);
        } else {
