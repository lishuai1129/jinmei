...
import { useChannelModelDataStore } from './channelModelData'
...
export const useTopoStore = defineStore('topo', {
  state: () => ({
    ...
    selectedChannelModel: null as string | null // 当前选中的信道模型类型
  }),
  ...
  actions: {
    ...
    // 设置当前选中的信道模型类型
    setSelectedChannelModel(model: string | null) {
      this.selectedChannelModel = model
    },
    ...
    async removeNodeRemote(nodeId: number) {
      ...
      // 删除节点后，清理channelModelDataStore中的对应数据
      const channelModelDataStore = useChannelModelDataStore();
      channelModelDataStore.clearNodeData(nodeId);
