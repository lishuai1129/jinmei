...
export function useCesiumEntities(viewer: Cesium.Viewer | null) {
      ...
      case 'RJ45':
        if (useSimplePoint) {
          ...
          if (node.name.includes('DRONE')) {
            ...
            // 判断是否为信道模型节点，根据name判断
            const isChannelModel = node.name && (
              node.name.includes('channel-gmsk') ||
              node.name.includes('channel-fhss') ||
              node.name.includes('channel-dsss') ||
              node.name.includes('channel-') // 通用信道模型前缀
            );
            ...
            entity = viewer.entities.add({
              ...
              model: {
                uri: isChannelModel ? '/models/emane.glb' : '/models/emane.glb', 
                minimumPixelSize: 32,
                maximumScale: isChannelModel ? 64 : 64,
                scale: isChannelModel ? 0.02 : 0.3,
                runAnimations: false,
                silhouetteColor: Cesium.Color.WHITE, 
                silhouetteSize: 3.0
              },
        ...
        if (link.type === 'WIRELESS') {
          ...
          viewer.entities.add({
            ...
            label: {
              ...
              fillColor: Cesium.Color.WHITE,
              outlineColor: Cesium.Color.BLACK,
              backgroundColor: Cesium.Color.fromCssColorString('#ff9500').withAlpha(0.5),
  ...
  const syncLinksToEntities = () => {
    ...
    emaneSubnets.forEach((nodeIds, subnetId) => {
      if (nodeIds.size >= 2) {
        ...
        // 【新逻辑】不再创建节点之间的直接链路，而是创建每个节点与子网的链路
        nodeIdsArray.forEach(nodeId => {
          // 创建 nodeId ↔ subnetId 的链路
          const linkId = `link-${Math.min(nodeId, subnetId)}-${Math.max(nodeId, subnetId)}`;
          requiredLinkIds.add(linkId);
        });
      }
    });
    ...
    emaneSubnets.forEach((nodeIds, subnetId) => {
      ...
      if (nodeIds.size >= 2) {
        ...
        // 【新逻辑】为每个网络节点创建与子网的虚线链路
        nodeIdsArray.forEach(nodeId => {
          const linkId = `link-${Math.min(nodeId, subnetId)}-${Math.max(nodeId, subnetId)}`;
          if (requiredLinkIds.has(linkId) && !linkEntities.value.has(linkId)) {
            const nodeEntity = nodeEntities.value.get(nodeId);
            const subnetEntity = nodeEntities.value.get(subnetId);
            const nodePos = nodeEntity?.position?.getValue(viewer.clock.currentTime);
            const subnetPos = subnetEntity?.position?.getValue(viewer.clock.currentTime);

            if (nodePos && subnetPos) {
  ...
  const createSubnetDashLink = (
    ...
    linkId: string // 使用普通链路ID格式
  ) => {
    // 使用传入的linkId，不再构建特殊的subnetLinkId
    const linkLabelId = `label-${linkId}`;
    ...
    // 检查实体是否已存在，防止重复创建
    const existingEntity = viewerInstance.entities.getById(linkId);
    if (existingEntity) {
    ...
    const dashEntity = viewerInstance.entities.add({
      ...
      polyline: {
        ...
        material: new Cesium.PolylineDashMaterialProperty({
          color: subnetColor.withAlpha(0.6),
          dashLength: 16.0
        }),
        clampToGround: false,
      }
    ...
    // 跟踪新的子网链路实体
    linkEntities.value.set(linkId, dashEntity);
    ...
    const labelEntity = viewerInstance.entities.add({
      ...
      label: {
        ...
        fillColor: Cesium.Color.WHITE,
        outlineColor: Cesium.Color.BLACK,
        backgroundColor: subnetColor.withAlpha(0.3),
        backgroundPadding: new Cesium.Cartesian2(10, 8),
        showBackground: true,
    ...
    if (node1Interface && (node1Interface.ip4 || node1Interface.ip6)) {
      ...
      viewerInstance.entities.add({
        ...
        label: {
          ...
          outlineColor: Cesium.Color.BLACK,
          fillColor: Cesium.Color.WHITE,
          backgroundColor: subnetColor.withAlpha(0.7),
          backgroundPadding: new Cesium.Cartesian2(10, 8),
          showBackground: true,
    ...
    try {
      ...
      for (let i = 0; i < entities.length; i++) {
        ...
        if (entity.properties?.isLinkDataLabel?.getValue()) {
          ...
          // 处理两种情况：
          // 1. 常规链路：node1和node2都有NEM信息
          // 2. 子网链路：只有node1（网络设备）有NEM信息，node2（EMANE节点）没有
          if (node1NemInfo?.ifaceNemMap && node2NemInfo?.ifaceNemMap) {
            ...
            if (node1NemId && node2NemId) {
              ...
              // 调整标签背景大小以适应内容
              if (entity.label.backgroundPadding) {
                entity.label.backgroundPadding = new Cesium.Cartesian2(10, 8);
              }

              // 根据SINR值更新链路颜色
              // 获取SINR数值（优先使用RF信号数据，否则使用指标数据）
              const sinrValue1to2 = rfData1to2 && Number.isFinite(rfData1to2.avgSINR)
                ? rfData1to2.avgSINR
                : (metricData1to2 && Number.isFinite(metricData1to2.sinrAvg)
                  ? metricData1to2.sinrAvg
                  : null);

              const sinrValue2to1 = rfData2to1 && Number.isFinite(rfData2to1.avgSINR)
                ? rfData2to1.avgSINR
                : (metricData2to1 && Number.isFinite(metricData2to1.sinrAvg)
              ...
              // 更新链路实体颜色
              const linkId = `link-${node1Id}-${node2Id}`;
          ...
          } else if (node1NemInfo?.ifaceNemMap && !node2NemInfo?.ifaceNemMap) {
            // 子网链路：只有node1（网络设备）有NEM信息，node2是EMANE节点
            // 获取node1的NEM ID来查询数据
            const getNemIdByInterfaceName = (ifaceNemMap: any, interfaceName: string | undefined): number | undefined => {
              if (!interfaceName) return undefined;
              if (ifaceNemMap[interfaceName] !== undefined) {
            ...
            if (node1NemId) {
              // 构建subnet链路的linkId
              const subnetLinkId = `link-${Math.min(node1Id, node2Id)}-${Math.max(node1Id, node2Id)}`;
              console.log('[子网链路标签更新]', 'linkId:', subnetLinkId, 'node1NemId:', node1NemId, 'node1Name:', node1Name, 'node2Name:', node2Name);

              // 对于子网链路，仅显示单向数据（device的信号信息）
              const shortName1 = node1Name.substring(0, 8);
              const shortName2 = node2Name.substring(0, 8);

              // 对于子网链路，只查询device节点的RF信号数据
              // node1NemId是设备的NEM ID，我们查询它的信号数据
              // 不指定viewpointNemId，因为EMANE节点本身不是一个有NEM ID的实体
              const rfData = emaneStore.getRFSignalByNem(node1NemId);
              const metricData = emaneStore.getMetricByNem(node1NemId);

              console.log('[子网链路数据查询]', 'node1NemId:', node1NemId, 'rfData:', rfData, 'metricData:', metricData);

              const sinr = rfData && Number.isFinite(rfData.avgSINR)
                ? `${rfData.avgSINR.toFixed(1)}dB`
                : (metricData && Number.isFinite(metricData.sinrAvg)
                  ? `${metricData.sinrAvg.toFixed(1)}dB`
                  : 'N/A');

              // 构建标签文本（仅显示信干噪比）
              labelText = `${shortName1}→${shortName2}\n信干噪比: ${sinr}`;

              console.log('[子网链路标签文本]', 'labelText:', labelText);

              // 根据SINR值更新链路颜色
              const sinrValue = rfData && Number.isFinite(rfData.avgSINR)
                ? rfData.avgSINR
                : (metricData && Number.isFinite(metricData.sinrAvg)
                  ? metricData.sinrAvg
                  : null);

              if (sinrValue !== null && entity.polyline) {
                let linkColor;
                if (sinrValue >= 15) {
                  linkColor = Cesium.Color.GREEN;
                } else if (sinrValue >= 5) {
                  linkColor = Cesium.Color.YELLOW;
                } else {
                  linkColor = Cesium.Color.RED;
                }

                if (entity.polyline.material instanceof Cesium.PolylineDashMaterialProperty) {
                  entity.polyline.material.color = new Cesium.ConstantProperty(linkColor.withAlpha(0.6));
                }
          ...
          // 关键修复：将构建的labelText实际分配给实体的label属性
          try {
            // console.log('[标签分配执行]', 'linkId:', entity.properties?.node1Id?.getValue() + '-' + entity.properties?.node2Id?.getValue(), 'labelText:', labelText.substring(0, 20));
            entity.label.text = new Cesium.ConstantProperty(labelText);
          } catch (e) {
            console.log('[标签分配错误]', e);
          }
        }
      }
    } catch (error) {
      ...
      if (selectedEntity && selectedEntity.id) {
        ...
        if (node) {
          if (node.type === 'DRONE' && nodeEntities.value.has(nodeId)) {
            const targetEntity = nodeEntities.value.get(nodeId);
            if (targetEntity) {
              ...
              // 创建独立的通信范围实体，而不是作为无人机的子组件
              // 这样可以确保位置实时更新，不受无人机实体位置更新延迟的影响
              const rangeEntityId = `range-${nodeId}`;
              ...
              const rangeEntity = viewer.entities.add({
                id: rangeEntityId,
                name: `${targetEntity.name || '无人机'} 通信范围`,
                position: new Cesium.CallbackProperty((time) => {
                  // 每次请求位置时，直接获取无人机当前位置
                  if (targetEntity && targetEntity.position) {
                    return targetEntity.position.getValue(time);
                  }
                  return undefined;
                }, false) as unknown as Cesium.PositionProperty,
                ellipsoid: {
              ...
              // 保存引用，但存储rangeEntity而不是targetEntity
              currentDroneWithRange.value = targetEntity;
              (targetEntity as any)._rangeEntity = rangeEntity;

              // 确保时钟在运行并设置正确的时钟参数
              if (viewer) {
                viewer.clock.shouldAnimate = true;
                viewer.clock.multiplier = 1.0; // 设置时钟速度
                viewer.clock.clockRange = Cesium.ClockRange.UNBOUNDED; // 允许时间无限前进
              }
            }
          } 
            ...
            if (targetEntity) {
              ...
              // 只有当节点配置存在且状态为RUNTIME时才显示干扰波束
              if (interferenceConfig && interferenceConfig.status === 'RUNTIME') {
                
                // 标记该实体正在显示波束
                if (targetEntity.properties) {
                  targetEntity.properties.isShowingBeam = true;
                }
              
              // 创建干扰波束
              createInterferenceBeam(nodeId, targetEntity);
