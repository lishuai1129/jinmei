...
const ensOptions = computed(() => {
  const options = [
    { label: 'ens37', value: 'ens37', disabled: false },
    { label: 'ens39', value: 'ens39', disabled: false },
    { label: 'ens40', value: 'ens40', disabled: false },
...
const generateUniqueNodeName = (typeCode: string, index?: number): string => {
  ...
  if (typeCode === 'RJ45') {
    ...
    if (!existingRJ45Names.includes('ens37')) {
      return 'ens37';
    } else if (!existingRJ45Names.includes('ens39')) {
      return 'ens39';
    } else if (!existingRJ45Names.includes('ens40')) {
      return 'ens40';
...
watch(
  ...
  (newPosition) => {
    if (newPosition) {
      ...
      const alt = Number(newPosition.alt);
