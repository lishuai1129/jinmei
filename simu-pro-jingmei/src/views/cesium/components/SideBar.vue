...
import { type Component, ref, computed, onMounted, watch, h } from 'vue';
import {
...
const getIconComponent = (iconName: string): Component => {
  const iconMap: Record<string, Component> = {
    ...
    'RadioStation': () => h('svg', {
      ...
      h('path', {
        d: 'M965 864.6H65V218.8h900v645.8z m-830-70h760V288.8H135v505.8zM588.6 117.8h232.3v70H588.6z',
        fill: 'currentColor'
      }),
      h('path', {
...
const channelModelTypeMap: Record<string, string> = {
  '测控链': 'TT_LINK',
  '协同链': 'COOP_LINK',
  '自组网': 'AD_HOC',
  'VHF': 'VHF',
...
const handleItemClick = (item: {name: string, icon: string, type?: string}, categoryType: string) => {
  if (categoryType === 'device' || categoryType === 'business') {
    ...
    const channelType = channelModelTypeMap[item.name] || 'CUSTOM';

    // @ts-ignore: 忽略类型检查错误，该方法确实存在
    topoStore.setOperationMode('add');
    // @ts-ignore: 忽略类型检查错误，该方法确实存在
    topoStore.setSelectedNodeType('EMANE');
    // @ts-ignore: 忽略类型检查错误，该方法确实存在
    topoStore.setSelectedChannelModel(channelType);

    item.type = 'EMANE';

    eventBus.emit('sidebar:channel-model-selected', { name: item.name, type: channelType });
    ...
    systemLogStore.addLog({
      ...
      action: '切换操作模式',
      information: '操作模式切换',
      details: `进入${item.name}信道模型放置模式`
    });

    ElMessage.success(`已进入${item.name}放置模式，点击地图放置信道模型节点，右键退出`);
...
const allCategories = [
  ...
  },  
  {
    ...
    type: 'channel',
    items: [
      { name: '测控链', icon: 'RadioStation' },
      { name: '协同链', icon: 'RadioStation' },
      { name: '自组网', icon: 'RadioStation' },
      { name: 'VHF', icon: 'RadioStation' },
      { name: 'UHF', icon: 'RadioStation' },
      { name: '5G通信', icon: 'RadioStation' },
      { name: '短波', icon: 'RadioStation' },
      { name: '中长波', icon: 'RadioStation' },
      { name: '卫星', icon: 'RadioStation' },
      { name: '散射', icon: 'RadioStation' },
    ]
  },
  {
    title: '链路管理',
