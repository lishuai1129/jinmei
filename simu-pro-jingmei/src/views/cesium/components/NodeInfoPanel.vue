<template>
  <div>
    ...
    <div class="neo-infobox" v-if="currentNode">
      ...
      <div class="neo-infobox-content">
        ...
        <!-- 频道模型配置显示 -->
        <div class="neo-section" v-if="isChannelModelNode && channelModelConfig">
          <div class="neo-section-title">
            <svg width="16" height="16" style="margin-right: 4px">
              <circle cx="8" cy="8" r="7" fill="#00bcd4" fill-opacity="0.18" />
              <circle cx="8" cy="8" r="4" fill="#00bcd4" fill-opacity="0.38" /></svg>信道模型配置 - {{ getChannelModelTypeLabel() }}
          </div>
          <div class="neo-resource-config">
            <!-- 基本配置 -->
            <div class="config-section">
              ...
              <div class="neo-grid">
                <!-- 模式 -->
                <div class="neo-grid-item" v-if="channelModelConfig.mode">
                  <div class="item-label">工作模式</div>
                  <div class="item-value">{{ channelModelConfig.mode === 'hopping' ? '跳频' : '固定频率' }}</div>
                </div>
                <!-- 中心频率 -->
                <div class="neo-grid-item" v-if="channelModelConfig.centerFreq !== undefined">
                  <div class="item-label">中心频率</div>
                  <div class="item-value">{{ channelModelConfig.centerFreq }} MHz</div>
                </div>
                <!-- 发射功率 -->
                <div class="neo-grid-item" v-if="channelModelConfig.txPower !== undefined">
                  <div class="item-label">发射功率</div>
                  <div class="item-value">{{ channelModelConfig.txPower }} dBm</div>
                </div>
                <!-- K因子 -->
                <div class="neo-grid-item" v-if="channelModelConfig.k_factor !== undefined">
                  <div class="item-label">K因子</div>
                  <div class="item-value">{{ channelModelConfig.k_factor }} dB</div>
                </div>
                <!-- 采样率 -->
                <div class="neo-grid-item" v-if="channelModelConfig.samplingRate">
                  <div class="item-label">采样率</div>
                  <div class="item-value">{{ channelModelConfig.samplingRate }} MHz</div>
                </div>
                <!-- 传输速率 -->
                <div class="neo-grid-item" v-if="channelModelConfig.dataRateKbps !== undefined">
                  <div class="item-label">传输速率</div>
                  <div class="item-value">{{ channelModelConfig.dataRateKbps }} Kbps</div>
                </div>
                <!-- 调制方式 -->
                <div class="neo-grid-item" v-if="channelModelConfig.modulation !== undefined">
                  <div class="item-label">调制方式</div>
                  <div class="item-value">{{ channelModelConfig.modulation }}</div>
                </div>
                <!-- 调制类型 -->
                <div class="neo-grid-item" v-if="channelModelConfig.modulation_type !== undefined">
                  <div class="item-label">调制类型</div>
                  <div class="item-value">{{ channelModelConfig.modulation_type }}</div>
                </div>
                <!-- SNR -->
                <div class="neo-grid-item" v-if="channelModelConfig.snr_db !== undefined">
                  <div class="item-label">信噪比</div>
                  <div class="item-value">{{ channelModelConfig.snr_db }} dB</div>
                </div>
              </div>
            </div>

            <!-- 多径配置 (Custom 模型) -->
            <div class="config-section" v-if="channelModelConfig.multipath && channelModelConfig.multipath.length > 0">
              <div class="config-section-title">多径配置 ({{ channelModelConfig.multipath.length }} 条路径)</div>
              <div class="multipath-list">
                <div class="multipath-item" v-for="(path, index) in channelModelConfig.multipath" :key="index">
                  <div class="path-header">路径 {{ index + 1 }}</div>
                  <div class="path-info">
                    <div class="info-row">
                      <span class="info-label">功率:</span>
                      <span class="info-value">{{ path.power_db }} dB</span>
                    </div>
                    <div class="info-row">
                      <span class="info-label">延迟:</span>
                      <span class="info-value">{{ path.delay_us }} μs</span>
                    </div>
                    <div class="info-row">
                      <span class="info-label">频偏:</span>
                      <span class="info-value">{{ path.freq_offset }} Hz</span>
                    </div>
                    <div class="info-row" v-if="channelModelConfig.type !== 'fiveG' && path.snr_db !== undefined">
                      <span class="info-label">SNR:</span>
                      <span class="info-value">{{ path.snr_db }} dB</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <!-- 已连接节点 -->
            <div class="config-section" v-if="channelModelConfig.connectedNodes && channelModelConfig.connectedNodes.length > 0">
              <div class="config-section-title">已连接节点 ({{ channelModelConfig.connectedNodes.length }})</div>
              <div class="connected-nodes-list">
                <div class="connected-node-item" v-for="connNode in channelModelConfig.connectedNodes" :key="connNode.id">
                  <div class="node-id">ID: {{ connNode.id }}</div>
                  <div class="node-name">名称: {{ connNode.name }}</div>
                  <div class="node-geo" v-if="connNode.geo">
                    位置: [{{ connNode.geo.lat.toFixed(4) }}, {{ connNode.geo.lon.toFixed(4) }}, {{ connNode.geo.alt.toFixed(1) }}m]
                  </div>
                </div>
              </div>
            </div>
        ...
        <!-- 连接到的频道模型 - 对所有节点显示 -->
        <div class="neo-section" v-if="connectedChannelModelConfigs.length > 0">
          <div class="neo-section-title">
            <svg width="16" height="16" style="margin-right: 4px">
              <circle cx="8" cy="8" r="7" fill="#9c27b0" fill-opacity="0.18" />
              <circle cx="8" cy="8" r="4" fill="#9c27b0" fill-opacity="0.38" />
            </svg>
            连接到的频道模型 ({{ connectedChannelModelConfigs.length }})
          </div>
          <div class="connected-models-container">
            <div class="connected-model-item" v-for="modelConfig in connectedChannelModelConfigs" :key="modelConfig.modelNodeId">
              <div class="model-header">
                <span class="model-label">{{ modelConfig.modelLabel }}</span>
                <span class="model-name">{{ modelConfig.modelNodeAlias || modelConfig.modelNodeName }}</span>
              </div>

              <!-- 频道模型的配置参数 -->
              <div class="model-config-grid">
                <div class="config-item" v-if="modelConfig.centerFreq !== undefined">
                  <span class="config-label">中心频率:</span>
                  <span class="config-value">{{ modelConfig.centerFreq }}</span>
                </div>
                <div class="config-item" v-if="modelConfig.txPower !== undefined">
                  <span class="config-label">发射功率:</span>
                  <span class="config-value">{{ modelConfig.txPower }} dBm</span>
                </div>
                <div class="config-item" v-if="modelConfig.k_factor !== undefined">
                  <span class="config-label">K因子:</span>
                  <span class="config-value">{{ modelConfig.k_factor }} dB</span>
                </div>
                <div class="config-item" v-if="modelConfig.samplingRate">
                  <span class="config-label">采样率:</span>
                  <span class="config-value">{{ modelConfig.samplingRate }}</span>
                </div>
                <div class="config-item" v-if="modelConfig.snr_db">
                  <span class="config-label">信噪比:</span>
                  <span class="config-value">{{ modelConfig.snr_db }}</span>
                </div>
                <div class="config-item" v-if="modelConfig.modulation !== undefined">
                  <span class="config-label">调制方式:</span>
                  <span class="config-value">{{ modelConfig.modulation }}</span>
                </div>
                <div class="config-item" v-if="modelConfig.mode">
                  <span class="config-label">工作模式:</span>
                  <span class="config-value">{{ modelConfig.mode }}</span>
                </div>
                <div class="config-item" v-if="modelConfig.adaptive !== undefined">
                  <span class="config-label">自适应:</span>
                  <span class="config-value">{{ modelConfig.adaptive }}</span>
                </div>
              </div>

              <!-- 多径配置 -->
              <div class="model-multipath" v-if="modelConfig.multipath && modelConfig.multipath.length > 0">
                <div class="multipath-title">多径配置 ({{ modelConfig.multipath.length }} 条路径)</div>
                <div class="multipath-items">
                  <div class="multipath-path" v-for="(path, index) in modelConfig.multipath" :key="index">
                    <div class="path-label">路径 {{ index + 1 }}</div>
                    <div class="path-details">
                      <span>功率: {{ path.power_db }} dB</span>
                      <span>延迟: {{ path.delay_us }} μs</span>
                      <span>频偏: {{ path.freq_offset }} Hz</span>
                      <span>SNR: {{ path.snr_db }} dB</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <!-- 信道模型节点（UHF/VHF/DSS等）：打开专属模型参数对话框 -->
        <div class="neo-actions" v-else-if="isSubnetNode && isChannelModelNode">
          <button class="neo-btn subnet-config-btn" @click="handleEditChannelModel">
      ...
      <div class="signal-sim-content">
        ...
        <div class="signal-sim-interfaces">
          <template v-if="nodeInterfaces.length > 0">
            <el-checkbox-group v-model="selectedInterfaces" class="interface-list">
              <el-checkbox 
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button 
            ...
            :disabled="selectedInterfaces.length === 0 || isLoading"
            :loading="isLoading"
          >
            {{ isLoading ? '处理中...' : '开启信号级仿真' }}
          </el-button>
        </div>
      </template>
    </el-dialog>
...
import { Delete, Monitor, Close, Minus, Connection, DataAnalysis, VideoPlay, View, Check, Loading, Key, Timer, Odometer, Upload, Download, Right, Document, SuccessFilled, CircleCloseFilled, InfoFilled, DocumentCopy, Refresh, Setting } from '@element-plus/icons-vue';
...
const emit = defineEmits(["close", "delete", "openTerminal", "openVnc", "editChannelModel"]);
...
// 识别频道模型类型
const getChannelModelType = (): string | null => {
  const node = currentNode.value;
  if (!node) return null;

  // 首先检查 phy_type 字段（最可靠的方式）
  const phyType = (node as any).phy_type;
  if (phyType) {
    const phyTypeMap: Record<string, string> = {
      'ttc': 'ttc',
      'adhoc': 'adhoc',
    ...
    if (phyTypeMap[phyType]) {
      console.log('[NodeInfoPanel] 通过 phy_type 检测到频道模型类型:', phyType);
      return phyTypeMap[phyType];
    }
  }

  // 其次检查节点名称模式
  if (node.name) {
    if (node.name.includes('channel-ttc-')) return 'ttc';
    if (node.name.includes('channel-adhoc-')) return 'adhoc';
    if (node.name.includes('channel-coordination-')) return 'coordination';
    if (node.name.includes('channel-custom-')) return 'custom';
    if (node.name.includes('channel-dss-')) return 'dss';
    if (node.name.includes('channel-fhss-')) return 'fhss';
    if (node.name.includes('channel-gmsk-')) return 'gmsk';
    if (node.name.includes('channel-gfsk-')) return 'gfsk';
    if (node.name.includes('channel-vhf-')) return 'vhf';
    if (node.name.includes('channel-uhf-')) return 'uhf';
    if (node.name.includes('channel-5g-')) return 'fiveG';
  }

  // 最后检查节点的config字段（备选方案）
  if (node.ttc_config) return 'ttc';
  if (node.adhoc_config) return 'adhoc';
  if (node.coordination_config) return 'coordination';
  if (node.custom_config) return 'custom';
  if (node.dss_config) return 'dss';
  if (node.fhss_config) return 'fhss';
  if (node.gmsk_config) return 'gmsk';
  if (node.gfsk_config) return 'gfsk';
  if (node.vhf_config) return 'vhf';
  if (node.uhf_config) return 'uhf';
  if (node.fiveG_config) return 'fiveG';

  return null;
};

// 检查是否是频道模型节点
const isChannelModelNode = computed(() => {
  const modelType = getChannelModelType();
  return modelType !== null;
});

// 获取连接到当前节点的频道模型配置
const connectedChannelModelConfigs = computed(() => {
  const connectedModels = getConnectedChannelModels();
  ...
  return connectedModels.map(model => {
    const config: any = {
      ...
      modelNodeName: model.modelNodeName,
      modelNodeAlias: model.modelNodeAlias
    };

    const runtimeData = model.runtimeData;
    const baseConfig = model.baseConfig;

    // 从持久化数据中提取配置
    if (runtimeData) {
      if (runtimeData.centerFreq !== undefined) {
        config.centerFreq = (runtimeData.centerFreq / 1000000).toFixed(2) + ' MHz';
      }
      if (runtimeData.txPower !== undefined) {
        config.txPower = runtimeData.txPower;
      }
      if (runtimeData.k_factor !== undefined) {
        config.k_factor = runtimeData.k_factor;
      }
      if (runtimeData.samplingRate !== undefined) {
        config.samplingRate = (runtimeData.samplingRate / 1000000).toFixed(2) + ' MHz';
      }
      if (runtimeData.snr_db !== undefined) {
        config.snr_db = runtimeData.snr_db + ' dB';
      }
      if (runtimeData.modulation !== undefined) {
        config.modulation = runtimeData.modulation;
      }
      if (runtimeData.mode !== undefined) {
        config.mode = runtimeData.mode;
      }
      if (runtimeData.adaptive !== undefined) {
        config.adaptive = runtimeData.adaptive ? '是' : '否';
      }
      if (runtimeData.multipath && runtimeData.multipath.length > 0) {
        config.multipath = runtimeData.multipath;
      }
    }

    // 回退到基础配置
    if (baseConfig) {
      if (config.centerFreq === undefined && baseConfig.centerFreq !== undefined) {
        const freq = baseConfig.centerFreq;
        if (freq > 1000000) {
          config.centerFreq = (freq / 1000000).toFixed(2) + ' MHz';
        } else {
          config.centerFreq = freq.toString() + ' MHz';
        }
      }
      if (config.txPower === undefined && baseConfig.txPower !== undefined) {
        config.txPower = baseConfig.txPower;
      }
      if (config.k_factor === undefined && baseConfig.k_factor !== undefined) {
        config.k_factor = baseConfig.k_factor;
      }
      if (config.samplingRate === undefined && baseConfig.samplingRate !== undefined) {
        const rate = baseConfig.samplingRate;
        if (rate > 1000000) {
          config.samplingRate = (rate / 1000000).toFixed(2) + ' MHz';
        } else {
          config.samplingRate = rate.toString() + ' MHz';
        }
      }
      if (config.snr_db === undefined && baseConfig.snr_db !== undefined) {
        config.snr_db = baseConfig.snr_db + ' dB';
      }
      if (!config.mode && baseConfig.mode !== undefined) {
        config.mode = baseConfig.mode;
      }
      if (config.adaptive === undefined && baseConfig.adaptive !== undefined) {
        config.adaptive = baseConfig.adaptive ? '是' : '否';
      }
      if (!config.multipath && baseConfig.multipath) {
        config.multipath = baseConfig.multipath;
      }
    }
...
// 获取频道模型配置
const channelModelConfig = computed(() => {
  const node = currentNode.value;
  const modelType = getChannelModelType();

  if (!modelType || !node) return null;

  const config: any = {};
  config.type = modelType;

  // 首先尝试从实时数据store获取配置数据
  const runtimeData = channelModelDataStore.getNodeData(node.id, modelType);
  console.log(`[NodeInfoPanel] Node ${node.id} (${modelType}) - 尝试获取运行时数据:`, {
    hasRuntimeData: !!runtimeData,
    runtimeData: runtimeData ? { ...runtimeData, timestamp: '...' } : null,
    baseConfigField: `${modelType}_config`,
    baseConfig: node[`${modelType}_config`]
  });
  ...
  if (runtimeData) {
    ...
    if (runtimeData.centerFreq !== undefined) {
      config.centerFreq = (runtimeData.centerFreq / 1000000).toFixed(2);
    }
    if (runtimeData.k_factor !== undefined) {
      config.k_factor = runtimeData.k_factor;
    }

    // PHY 配置（用于包含 multipath 的模型）
    if (runtimeData.phy) {
      const phy = runtimeData.phy;
      if (phy.txPower !== undefined) {
        config.txPower = phy.txPower;
      }
      if (phy.centerFreq !== undefined) {
        config.centerFreq = (phy.centerFreq / 1000000).toFixed(2);
      }
      if (phy.k_factor !== undefined) {
        config.k_factor = phy.k_factor;
      }
      if (phy.speed !== undefined) {
        config.speed = phy.speed;
      }
      if (phy.samplingRate) {
        config.samplingRate = (phy.samplingRate / 1000000).toFixed(2);
      }
      if (phy.snr_db !== undefined) {
        config.snr_db = phy.snr_db;
      }
      if (phy.multipath && phy.multipath.length > 0) {
        config.multipath = phy.multipath;
      }
    }

    // 工作模式（mode 用于 TTC/AdHoc/Coordination）
    if (runtimeData.mode) {
      config.mode = runtimeData.mode;
    }
    ...
    // 自适应（用于需要的模型）
    if (runtimeData.adaptive !== undefined) {
      config.adaptive = runtimeData.adaptive;
    }

    // 调制方式（modulation 或 modulation_type）
    if (runtimeData.modulation !== undefined) {
      config.modulation = runtimeData.modulation;
    }
    if (runtimeData.modulation_type !== undefined) {
      config.modulation_type = runtimeData.modulation_type;
    }

    // 散射的采样率和背景SNR
    if (runtimeData.samplingRate) {
      config.samplingRate = (runtimeData.samplingRate / 1000000).toFixed(2);
    }
    if (runtimeData.snr_db !== undefined) {
      config.snr_db = runtimeData.snr_db;
    }
    if (runtimeData.dataRateKbps !== undefined) {
      config.dataRateKbps = runtimeData.dataRateKbps;
    }

    // 多径配置（Custom 模型直接存储在 runtimeData 中，不在 phy 下）
    if (runtimeData.multipath && runtimeData.multipath.length > 0) {
      config.multipath = runtimeData.multipath;
      console.log(`[NodeInfoPanel] 从runtimeData中提取multipath，路径数: ${runtimeData.multipath.length}`, runtimeData.multipath);
    }
  }

  // 回退到节点基本配置
  const configField = `${modelType}_config`;
  const baseConfig = node[configField];

  if (baseConfig) {
    // 为了DSSS和5G，确保从baseConfig中提取modulation_type
    if (baseConfig.modulation !== undefined && !config.modulation) {
      config.modulation = baseConfig.modulation;
    }
    if (baseConfig.modulation_type !== undefined && !config.modulation_type) {
      config.modulation_type = baseConfig.modulation_type;
    }

    // 如果runtimeData没有提供这些值，则从baseConfig中提取
    if (!config.centerFreq && baseConfig.centerFreq) {
      // baseConfig中的centerFreq可能是Hz（如果是persisted数据）或MHz（如果是node原始数据）
      // 需要根据大小判断
      const centerFreqValue = baseConfig.centerFreq;
      if (centerFreqValue > 1000000) {
        // 如果大于1MHz，说明是Hz，需要转换为MHz
        config.centerFreq = (centerFreqValue / 1000000).toFixed(2);
      } else {
        // 否则就是MHz
        config.centerFreq = centerFreqValue.toString();
      }
    }
    if (config.txPower === undefined && baseConfig.txPower !== undefined) {
      config.txPower = baseConfig.txPower;
    ...
    if (!config.mode && baseConfig.mode) {
      config.mode = baseConfig.mode;
    }
    if (config.adaptive === undefined && baseConfig.adaptive !== undefined) {
      config.adaptive = baseConfig.adaptive;
    }
    if (!config.samplingRate && baseConfig.samplingRate) {
      config.samplingRate = (baseConfig.samplingRate / 1000000).toFixed(2);
    }
    if (config.snr_db === undefined && baseConfig.snr_db !== undefined) {
      config.snr_db = baseConfig.snr_db;
    }
    if (!config.multipath && baseConfig.multipath) {
      config.multipath = baseConfig.multipath;
    }
  ...
  // 获取传输延迟配置
  const transmissionDelay = transmissionConfigStore.getNodeTransmissionDelay(node.id);
  if (transmissionDelay !== undefined) {
    config.transmissionDelay = transmissionDelay;
  }

  // 获取编码配置
  const codingConfig = codingConfigStore.getNodeCodingConfig(node.id);
  if (codingConfig) {
    config.codingConfig = codingConfig;
  }

  // 获取已连接的节点
  const connectedNodesList = getConnectedNodes();
  if (connectedNodesList.length > 0) {
    config.connectedNodes = connectedNodesList;
  }

  console.log(`[NodeInfoPanel] Final config for node ${node.id}:`, config);
  return config;
});
...
const getConnectedNodes = () => {
  ...
  if (!node || !modelType) return [];

  let service: any = null;
  switch (modelType) {
    case 'ttc':
      service = ttcSimulationService;
      break;
    case 'adhoc':
      service = adhocSimulationService;
      break;
    case 'coordination':
      service = coordinationSimulationService;
      break;
    case 'custom':
      service = customSimulationService;
      break;
    case 'dss':
      service = dssSimulationService;
      break;
    case 'fhss':
      service = fhssSimulationService;
      break;
    case 'gmsk':
      service = gmskSimulationService;
      break;
    case 'gfsk':
      service = gfskSimulationService;
      break;
    case 'vhf':
      service = vhfSimulationService;
      break;
    case 'uhf':
      service = uhfSimulationService;
      break;
    case 'fiveG':
      service = fiveGSimulationService;
      break;
    default:
      return [];
  }
  ...
  // 获取服务中注册的连接节点
  // 使用公开的getter方法获取连接节点映射
  let connectedNodesMap: Map<number, any[]> | undefined = undefined;

  if (typeof (service as any).getConnectedNodes === 'function') {
    connectedNodesMap = (service as any).getConnectedNodes();
  } else if ((service as any).connectedNodes) {
    // 备选方案：直接访问私有属性（用于向后兼容）
    connectedNodesMap = (service as any).connectedNodes;
  }

  if (!connectedNodesMap || !connectedNodesMap.has(node.id)) {
    return [];
  }

  return connectedNodesMap.get(node.id) || [];
};

// 获取连接到当前节点的所有频道模型
const getConnectedChannelModels = () => {
  const node = currentNode.value;
  if (!node) return [];

  const connectedModels: any[] = [];
  const modelServices = [
    { name: 'custom', label: '散射', service: customSimulationService },
    { name: 'ttc', label: '测控链', service: ttcSimulationService },
    { name: 'adhoc', label: '自组网', service: adhocSimulationService },
    { name: 'coordination', label: '协同链', service: coordinationSimulationService },
    { name: 'dss', label: 'DSSS', service: dssSimulationService },
    { name: 'fhss', label: 'FHSS', service: fhssSimulationService },
    { name: 'gmsk', label: 'GMSK', service: gmskSimulationService },
    { name: 'gfsk', label: 'GFSK', service: gfskSimulationService },
    { name: 'vhf', label: 'VHF', service: vhfSimulationService },
    { name: 'uhf', label: 'UHF', service: uhfSimulationService },
    { name: 'fiveG', label: '5G', service: fiveGSimulationService },
  ];

  // 遍历所有频道模型服务
  for (const modelInfo of modelServices) {
    const service = modelInfo.service;
    ...
    if (!connectedNodesMap) continue;

    // 检查当前节点是否在任何频道模型的连接节点中
    for (const [channelModelNodeId, connectedNodes] of connectedNodesMap.entries()) {
      if (connectedNodes.some((cn: any) => cn.id === node.id)) {
        // 找到了一个连接到当前节点的频道模型
        const topoStore = useTopoStore();
        const channelModelNode = topoStore.topoData?.nodes?.find((n: any) => n.id === channelModelNodeId);

        if (channelModelNode) {
          // 获取该频道模型的配置
          const runtimeData = channelModelDataStore.getNodeData(channelModelNodeId, modelInfo.name);
          const configFieldName = `${modelInfo.name}_config`;
          const baseConfig = channelModelNode[configFieldName];

          connectedModels.push({
            modelType: modelInfo.name,
            modelLabel: modelInfo.label,
            modelNodeId: channelModelNodeId,
            modelNodeName: channelModelNode.name,
            modelNodeAlias: channelModelNode.alias,
...
// 获取频道模型类型的中文标签
const getChannelModelTypeLabel = (): string => {
  const modelType = getChannelModelType();
...
const getNodeTypeLabel = (type: string): string => {
  ...
  // 半实物节点
  if (nodeName.startsWith('ens37') || nodeName.startsWith('半实物')) {
    return '半实物节点';
  }
...
// 处理信道模型参数修改（打开模型专属对话框）
const handleEditChannelModel = () => {
  if (!currentNode.value) return;
  if (isSimulationRunning.value) {
    ElMessage.warning('仿真运行时无法修改参数，请先停止仿真');
    return;
  }
  emit('editChannelModel', currentNode.value);
};
...
.connected-model-item .model-header {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 10px;
...
.connected-model-item .model-label {
...
.connected-model-item .model-name {
  color: #ba68c8;
  font-weight: 500;
  flex: 1;
  word-break: break-all;
...
.model-multipath .multipath-title {
  color: #4caf50;
  font-weight: 600;
  font-size: 11px;
  margin-bottom: 8px;
